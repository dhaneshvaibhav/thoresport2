        ...
        <div style={styles.createTeamSection}>
          ...
          {myTeams.length > 0 && (
            <div style={styles.myTeamsBox}>
              <h3 style={{ textAlign: 'center', fontFamily: 'Orbitron', color: 'lightblue' }}>My Teams</h3>
              <ul style={{ listStyle: 'none', padding: 0 }}>
                {myTeams.map(team => (
                  <li key={team.id} style={{ marginBottom: 16 }}>
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12 }}>
                      {team.teams?.team_logo_url && (
                        <img src={team.teams.team_logo_url} alt="logo" width={32} height={32} style={{ borderRadius: 4 }} />
                      )}
                      <b style={{ color: '#01E2E9' }}>{team.teams?.team_name}</b>
                      {team.is_captain
                        ? <span style={{ marginLeft: 8, color: '#1976d2', fontWeight: 600 }}>(Captain)</span>
                        : null
                      }
                      {team.is_captain ? (
                        <button style={{ marginLeft: 16 }} onClick={() => { setEditTeamId(team.team_id); setShowEditModal(true); }}>
                          Edit Team
                        </button>
                      ) : (
                        <button style={{ marginLeft: 16 }} onClick={() => leaveTeam(team)}>
                          Leave Team
                        </button>
                      )}
                    </div>
                    {teamMembers[team.team_id]?.length > 0 && (
                      <ul style={{ listStyle: 'none', padding: 0, textAlign: 'center', marginTop: 6 }}>
                        {teamMembers[team.team_id].map(m => (
                          <li key={m.id}>
                            {m.profiles?.username || m.profiles?.email || m.user_id}
                            {m.is_captain && <span style={{ marginLeft: 4, color: '#1976d2' }}>(Captain)</span>}
                          </li>
                        ))}
                      </ul>
                    )}
              ...
              {tournaments.map(t => (
                <div key={t.id} style={styles.tournamentCard}>
                  ...
                  <div style={styles.tournamentButtonGroup}>
                    <button onClick={() => { setRegisterTournament(t); setShowRegisterModal(true); }}>Join</button>
                    <button onClick={() => navigate(`/tournament/${t.id}`)}>View More</button>
                  </div>
                </div>
              ))}
            </div>
